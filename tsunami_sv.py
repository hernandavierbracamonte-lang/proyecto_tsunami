...
# Fondo
fondo = pygame.image.load("assets/img/ciudad14.jpg") 
...
# Escalar fondo de game over y menú
...
class Jugador(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.suelo_y = ALTO - 50  # Posición límite donde camina
    ...
    def laser(self):
        if self.direccion == "DERECHA":
            rayo = Balas(self.rect.right - 30, self.rect.centery - 10)
        else: 
            rayo = Balas_V(self.rect.left + 30, self.rect.centery - 10)
    ...
    def update(self):
        # Manejo de animación cuando recibe golpe
        if self.timer_golpe > 0:
            self.timer_golpe -= 1
            if self.timer_golpe == 0:
                self.image = self.tsunami_V if self.direccion == "IZQUIERDA" else self.tsunami_A
